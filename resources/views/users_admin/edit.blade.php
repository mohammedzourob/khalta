...
{{--                </div>--}}
                ...
                {!! Form::model($user, ['method' => 'PATCH','route' => ['users_admin.update', $user->id]]) !!}
                <div class="">
                ...
                <div class="row row-sm mg-b-20">
                    <div class="col-lg-6">
                        ...
                        <select name="Status" id="select-beast" class="form-control  nice-select  custom-select">
                            @if($user->Status == 0)
                            <option value="{{ $user->Status}}">{{trans('users_admin.Inactive')}}</option>
                            @elseif($user->Status == 1)
                                <option value="{{ $user->Status}}">{{trans('users_admin.active')}}</option>
                             @endif
